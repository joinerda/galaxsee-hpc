...
int main(int argc, char ** argv) {
    ...
    while (!done) {
        if(first_time) {
            ...
            setMassNbodyModel(theModel,1.0/(double)n); // solar masses
            setGNbodyModel(theModel,0.00001); // pc^3/solar_mass/My^2
          ...
          if(rank==0) {
            ...
            if((count_updates++)%skip_updates==0&&show_display) {
                ...
                updateNbodyModel(theModel,UPDATEMETHOD_HASH_TEXT);
            }
#ifdef HAS_MPI
          }
#endif
